...
const enabledIntegrationIds = computed(() =>
  (integrations.value as Integration[])
    .filter(i => i.enabled)
    .map(i => i.id),
);

watch(enabledIntegrationIds, (enabledIds) => {
...
const availableCalendars = computed(() => {
  ...
  for (const integration of allIntegrations) {
    if (integration.service === "google-calendar" && integration.enabled && integration.settings) {
      ...
      let settings: any = integration.settings;
      ...
      const metadata = settings.calendarMetadata || {};
      const selectedIds = Array.isArray(settings.selectedCalendars)
