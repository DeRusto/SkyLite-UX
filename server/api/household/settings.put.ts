...
export default defineEventHandler(async (event) => {
  ...
  let parentPinHash: string | null | undefined;

  if (body.parentPin !== undefined) {
    if (body.parentPin && body.parentPin.trim() !== "") {
      parentPinHash = await hashPin(body.parentPin);
    }
    else {
      parentPinHash = null;
    }
  ...
  const updatedSettings = await prisma.householdSettings.update({
    ...
    data: {
      ...
      ...(parentPinHash !== undefined && { parentPin: parentPinHash }),
    },
  });
